...
import { Settings, TrendingUp, AlertTriangle, CheckCircle, Wifi, ChevronUp, ChevronDown } from 'lucide-react';
...
const WiFiOptimizer = () => {
    ...
    // Charger l'état de réduction depuis le cookie au montage
    useEffect(() => {
        const savedCollapsed = getCookie('wifiOptimizerCollapsed');
        if (savedCollapsed !== null) {
            setIsCollapsed(savedCollapsed === 'true');
        }
    }, []);

    // Sauvegarder l'état de réduction dans le cookie
    const toggleCollapsed = () => {
        const newCollapsed = !isCollapsed;
        setIsCollapsed(newCollapsed);
        setCookie('wifiOptimizerCollapsed', newCollapsed.toString());
    };
    ...
    if (loading) {
        return (
            <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                {!isCollapsed && <p className="text-gray-600">Analyse en cours...</p>}
            </div>
        );
    }
    ...
    if (!optimizationData) {
        return (
            <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                {!isCollapsed && <p className="text-gray-600">Aucune donnée d'optimisation disponible</p>}
            </div>
        );
    }
